// 生僻字注音：使用《现代汉语常用字表》（1988）2500 字作为密度边界。
// 原文中的古读另行覆盖；AI 输出只使用表外字的普通读音，避免把白话“见”误读为 xiàn。

export type AnnotationMode = "classical" | "modern";

// 《现代汉语常用字表》（1988）常用字（2500 字），按笔画顺序保存以便轻量打包。
export const COMMON_2500 = new Set(
...
// 《周易》原文中表外字的人工读音表。AI 输出也只从这张小表中取音，未知字不擅自标注。
...
// 原文中的常用字也可能有《周易》古读；这些只在 classical 模式启用。
const CLASSICAL_PINYIN: Record<string, string> = {
  乾: "qián",
  见: "xiàn",
  否: "pǐ",
  屯: "zhūn",
...
export function shouldAnnotate(
  ...
  return Boolean(getPinyin(char, mode));
}
