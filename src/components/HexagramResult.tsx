...
export function HexagramResult({ result }: { result: CastResult }) {
    ...
    <section className="fade-in-up grid gap-4 md:grid-cols-2">
      ...
      <div className="rounded-2xl border border-[var(--line)] bg-[var(--paper)] p-5 shadow-sm">
        <div className="mb-3 flex justify-center">
          <HexagramFigure lines={yaos.map((y) => ({ yang: y.yang, moving: y.moving }))} />
        </div>
        <div className="mb-3 text-center">
          <div className="flex items-center justify-center gap-2">
            ...
            <h3 className="font-serif-cn text-2xl font-bold text-[var(--ink)]">{original.name}</h3>
          </div>
          <p className="mt-1 text-sm text-[var(--ink-soft)]">
            {original.image} · {original.lower}下{original.upper}上
          </p>
        </div>

        <div className="rounded-xl bg-[var(--bg)]/60 p-3">
          <div className="mb-1 text-xs text-[var(--gold)]">卦辞</div>
          <p className="font-serif-cn text-[0.95rem] leading-relaxed">{original.guaci}</p>
        </div>
        ...
        {isStatic ? (
          ...
          <div className="mt-3">
            <div className="mb-1.5 text-xs text-[var(--gold)]">动爻爻辞（{moving.length} 爻动）</div>
      ...
      {changed && (
        <div className="fade-in-up rounded-2xl border border-[var(--line)] bg-[var(--paper)] p-5 shadow-sm">
          <div className="mb-3 flex justify-center">
            <HexagramFigure lines={changed.bits.map((b) => ({ yang: b === 1 }))} />
          </div>
          <div className="mb-3 text-center">
            <div className="flex items-center justify-center gap-2">
              ...
              <h3 className="font-serif-cn text-2xl font-bold text-[var(--ink)]">{changed.name}</h3>
            </div>
            <p className="mt-1 text-sm text-[var(--ink-soft)]">
              {changed.image} · {changed.lower}下{changed.upper}上
            </p>
          </div>

          <div className="rounded-xl bg-[var(--bg)]/60 p-3">
            <div className="mb-1 text-xs text-[var(--gold)]">变卦卦辞</div>
            <p className="font-serif-cn text-[0.95rem] leading-relaxed">{changed.guaci}</p>
          </div>
    ...
    <li className="flex items-start gap-2 rounded-lg bg-[var(--bg)]/60 px-3 py-2">
      <span
        className={`mt-0.5 shrink-0 rounded px-1.5 py-0.5 text-xs font-bold ${
          badgeTone === "move" ? "bg-[var(--accent)]/10 text-[var(--accent)]" : "bg-[var(--gold)]/15 text-[var(--gold)]"
        }`}
      >
        {badge}
      </span>
      <span className="font-serif-cn text-sm leading-relaxed">
        <b className="mr-1 font-bold text-[var(--ink)]">{name}</b>
        {text}
      </span>
    </li>
  );
